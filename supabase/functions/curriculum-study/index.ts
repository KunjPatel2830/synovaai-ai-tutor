...
NUMERICAL CONSISTENCY — CRITICAL: In worked examples or calculations, solve from the stated values only, recompute the arithmetic once silently, and make sure the final value exactly matches the shown steps and units.
...
NUMERICAL CONSISTENCY — CRITICAL: For numerical doubts, extract given values first, solve from them only, recompute arithmetic once silently, and output only the verified final answer. If ambiguous, state one assumption and keep the answer consistent with it.`;
      userPrompt =
        messages.length > 0
    ...
    if (lovableResp.ok) {
      ...
      const choice = lovableJson.choices?.[0];
      if (choice?.finish_reason === "length") {
        return jsonResponse({ error: "AI response was cut off. Please try again with a shorter request." }, { status: 502 });
      }
