...
serve(async (req) => {
  ...
  try {
    ...
    const { pdfBase64, examType, year, shift, fileName } = body;
    ...
    if (!uploadId) {
      console.log(`[parse-pyq-pdf] Creating pyq_uploads record internally for user: ${authenticatedUserId}`);
      ...
      // If retrying existing upload, ensure it belongs to authenticated user
      const { data: existingUpload, error: checkErr } = await supabase
        .from("pyq_uploads")
        .select("id")
        .eq("id", uploadId)
