...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "load-data") {
      ...
      if (!isParticipant) return jsonRes({ error: "Not a participant" }, 403);
...
- For numerical questions, extract values first, solve from them only, recompute arithmetic once silently, and output only the verified final answer. Same question = same answer.
...
${chatContext ? `Recent chat:\n${chatContext}` : ""}`;
      ...
      try {
        ...
        if (aiData.choices?.[0]?.finish_reason === "length") {
          return jsonRes({ error: "AI response was cut off. Please ask a shorter question." }, 502);
        }
