...
serve(async (req) => {
  ...
  try {
    ...
    const { data: rl } = await admin.rpc("check_rate_limit", { _user_id: auth.userId, _endpoint: "ai-tutor", _max_requests: 20, _window_seconds: 60 });
...
NUMERICAL CONSISTENCY — CRITICAL:
- For any math/physics/chemistry numerical question, the SAME input must lead to the SAME final answer every time.
- Extract the given values first, then solve from those values only. Do not invent missing numbers.
- Independently recompute the final arithmetic once before answering. If the two calculations disagree, correct the working before writing the final answer.
- Keep units through every substitution and final answer.
- If the question is ambiguous, state the single assumption you are using and keep the final answer consistent with that assumption.
...
- Use [IMAGE: concept description] for visual concepts`;
    ...
    if (choice?.finish_reason === "length") {
      return jsonResponse({ error: "AI response was cut off. Please ask the question again in a shorter form." }, { status: 502 });
    }
