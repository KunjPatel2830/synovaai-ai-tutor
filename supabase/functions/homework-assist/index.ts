...
- For any numerical problem, solve from the extracted given values only.
- The same question must not get two different results across retries.
- Recompute arithmetic once silently before responding. If the check disagrees, correct the step before replying.
- Keep units attached to every substituted value.
